...
import { DonorChatBox } from "./DonorChatBox";
    ...
    <section className="rounded-3xl border border-emerald-100 bg-white p-6 shadow-[0_18px_60px_rgba(8,39,24,0.1)] sm:p-8">
      ...
      <form onSubmit={onSubmit} className="grid gap-3 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4 sm:grid-cols-3">
        <label className="space-y-1">
          ...
          <select value={bloodGroup} onChange={(event) => setBloodGroup(event.target.value)} className="w-full rounded-xl border border-emerald-200 px-3 py-2 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200">
        ...
        <label className="space-y-1">
          ...
          <input required value={city} onChange={(event) => setCity(event.target.value)} className="w-full rounded-xl border border-emerald-200 px-3 py-2 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" placeholder="Bahawalpur" />
        </label>

        <div className="flex items-end">
          <button type="submit" disabled={isSubmitting} className="w-full rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-60">{isSubmitting ? "Matching..." : "Find matched donors"}</button>
        </div>
      </form>
      ...
      <div className="mt-5 grid gap-3">
        {matches.length === 0 ? (
          <p className="rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">No ranked donors yet. Run an emergency request to view smart matches.</p>
        ) : (
          matches.map((donor, index) => (
            <article key={donor.id} className="rounded-2xl border border-emerald-100 bg-white p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h3 className="text-base font-semibold text-emerald-950">#{index + 1} {donor.name}</h3>
                <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-800">Score {donor.score}</span>
              </div>
              <p className="mt-2 text-sm text-emerald-900/80">{donor.bloodGroup || "N/A"} | {donor.city || "N/A"} | {donor.availabilityStatus}</p>
