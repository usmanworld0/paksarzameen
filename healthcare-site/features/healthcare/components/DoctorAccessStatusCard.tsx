...
export function DoctorAccessStatusCard({ userEmail, request }: { userEmail: string; request: any | null }) {
      ...
      <section className="mx-auto max-w-3xl space-y-6">
        <div className="rounded-3xl border border-emerald-100 bg-white p-6 shadow-sm sm:p-8">
          ...
          <h1 className="mt-3 text-3xl font-semibold text-slate-900">
            {isPending ? "Your application is under review." : isDeclined ? "Your application was declined." : "Doctor access has not been requested yet."}
          </h1>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {!isPending ? (
              <Link href="/healthcare/doctor/sign-up" className="inline-flex items-center justify-center rounded-full bg-emerald-700 px-5 py-2.5 text-sm font-semibold text-white">
                {isDeclined ? "Update and Resubmit" : "Apply as Doctor"}
              </Link>
            ) : null}
            <Link href="/healthcare/doctor/sign-in" className="inline-flex items-center justify-center rounded-full border border-slate-300 px-5 py-2.5 text-sm font-semibold text-slate-700">Go to Sign In</Link>
