  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-xl space-y-5 rounded-3xl border border-emerald-100 bg-white p-7 shadow-[0_24px_90px_rgba(7,41,25,0.12)] sm:p-9">
      <div className="space-y-1">
        ...
        <p className="text-sm text-emerald-900/70">Sign in with your doctor account. Approved accounts go straight to the doctor dashboard.</p>
      </div>
      ...
      <button type="submit" disabled={isSubmitting} className="w-full rounded-xl bg-emerald-700 px-4 py-3 text-sm font-semibold text-white transition hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-60">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-sm text-emerald-900/80">
        <Link href="/healthcare/doctor/sign-up" className="font-semibold text-emerald-700 hover:text-emerald-800">Need a doctor account?</Link>
      ...
      {supabaseError ? (
        <p className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">{supabaseError}</p>
      ) : null}
    </form>
  );
}
