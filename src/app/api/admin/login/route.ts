...
import {
  ...
  normalizeAdminSessionModuleName,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (hasSupabaseServiceRoleKey()) {
      ...
      if (isFallbackAdminEmail || resolvedRole === "admin") {
        ...
        for (const permission of permissions ?? []) {
          const moduleName = normalizeAdminSessionModuleName(permission.module);
          if (!moduleName) continue;

          const existing = permissionMap.get(moduleName);
          permissionMap.set(moduleName, {
            module: moduleName,
            can_view: Boolean(existing?.can_view || permission.can_view),
            can_edit: Boolean(existing?.can_edit || permission.can_edit),
            can_manage: Boolean(existing?.can_manage || permission.can_manage),
          });
        }

        sessionPermissions = Array.from(permissionMap.values()).filter(
    ...
    if (sessionRole === "tenant" && redirectTo === "/admin/login") {
      return NextResponse.json(
        { error: "This tenant account has no supported module access." },
        { status: 403 }
      );
    }
