...
export function CategoryCarousel({ categoryImages }: CategoryCarouselProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      setPrevTranslate((prev) => {
        let newTranslate = prev - 2;
        const trackWidth = 50 * (getCardWidth() + 16);
  ...
  const renderCard = (category: string, keySuffix: string) => (
    <Link
      key={`${category}${keySuffix}`}
      href={`/commonwealth-lab/products?category=${encodeURIComponent(category)}`}
      className="category-card group relative flex-shrink-0 aspect-[3/4] items-end overflow-hidden rounded-2xl border border-[#E5E5E5] bg-neutral-900"
    ...
    >
      ...
      <div className="relative z-10 p-6 w-full">
        <h3 className="text-base font-black uppercase tracking-tighter text-white pointer-events-none">
          {category}
        </h3>
        <p className="mt-1.5 text-[10px] font-semibold uppercase tracking-[0.22em] text-white/50 transition-colors duration-300 group-hover:text-[#0f7a47] pointer-events-none">
          Explore →
        </p>
      </div>
  ...
  return (
    ...
    >
      ...
      >
        {/* First set */}
        {PRODUCT_CATEGORIES.map((category) => renderCard(category, ""))}
        {/* Second set (for seamless loop) */}
        {PRODUCT_CATEGORIES.map((category) => renderCard(category, "-2"))}
      </div>
    </div>
  );
}
