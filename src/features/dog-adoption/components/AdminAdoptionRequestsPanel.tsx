                  ...
                  {rows.map((row) => (
                    <tr key={row.requestId} className="border-t border-slate-100">
                      <td className="px-3 py-2">
                        ...
                        <p className="text-xs text-slate-500">{row.dogColor}</p>
                        {row.petName ? <p className="text-xs text-indigo-700">Pet name: {row.petName}</p> : null}
                      </td>
                      <td className="px-3 py-2">
                        <p className="text-slate-700">{row.applicantName ?? row.userName ?? "Unknown"}</p>
                        <p className="text-xs text-slate-500">{row.applicantPhone ?? row.userEmail ?? row.userId ?? "—"}</p>
                      </td>
                      <td className="px-3 py-2 text-slate-700">{row.applicantPhone ?? row.whatsappNumber ?? "N/A"}</td>
