...
export function normalizeAdminSessionModuleName(moduleName: string) {
  ...
  if (normalized === "blood" || normalized === "bloodbank") {
    return "blood_bank";
  }
  if (normalized === "dog" || normalized === "dogs" || normalized === "adoption") {
    return "dog_adoption";
  }
  if (normalized === "health" || normalized === "health_care") {
    return "healthcare";
  }
...
export function hasAdminSessionPermission(
  ...
  const normalizedModuleName = normalizeAdminSessionModuleName(moduleName);
  const permission = session.permissions.find(
    (entry) => normalizeAdminSessionModuleName(entry.module) === normalizedModuleName
  );
  if (!permission) return false;

  if (action === "view") return Boolean(permission.can_view || permission.can_edit || permission.can_manage);
  if (action === "edit") return Boolean(permission.can_edit || permission.can_manage);
