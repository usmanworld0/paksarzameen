...
// Action inheritance applied hierarchically below.
...
export async function hasTenantPermission(
  ...
  try {
    ...
    if (action === "view") return Boolean(data.can_view || data.can_edit || data.can_manage);
    if (action === "edit") return Boolean(data.can_edit || data.can_manage);
